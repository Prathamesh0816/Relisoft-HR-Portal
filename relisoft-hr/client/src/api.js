...
export async function updateAsset(id, req) {
  const { data } = await api.put(`/api/assets/${id}`, req)
  return data
}
...
export async function getAssetTemplate() {
  const res = await api.get('/api/excel/asset-template', { responseType: 'blob' })
  return res.data
}

export async function uploadAssetExcel(formData) {
  const { data } = await api.post('/api/excel/upload-assets', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
  return data
}
...
// ─── HR Policy Documents ───
export async function getHrPolicyDocs(category, search) {
  const params = {}
  if (category) params.category = category
  if (search) params.search = search
  const { data } = await api.get('/api/hr-policy-docs', { params })
  return data
}
export async function getHrPolicyDoc(id) {
  const { data } = await api.get(`/api/hr-policy-docs/${id}`)
  return data
}
export async function uploadHrPolicyDoc(formData) {
  const { data } = await api.post('/api/hr-policy-docs', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
  return data
}
export async function updateHrPolicyDoc(id, req) {
  const { data } = await api.put(`/api/hr-policy-docs/${id}`, req)
  return data
}
export async function deleteHrPolicyDoc(id) {
  const { data } = await api.delete(`/api/hr-policy-docs/${id}`)
  return data
}
export async function downloadHrPolicyDoc(id) {
  const res = await api.get(`/api/hr-policy-docs/${id}/download`, { responseType: 'blob' })
  return res.data
}
export async function getHrPolicyCategories() {
  const { data } = await api.get('/api/hr-policy-docs/categories')
  return data
}
...
export async function getGatePass(visitorId) {
  const res = await api.get(`/api/phase2/gate-pass/${visitorId}`, { responseType: 'text' })
  return res.data
}
