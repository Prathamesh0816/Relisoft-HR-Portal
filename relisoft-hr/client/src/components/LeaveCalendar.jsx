...
export default function LeaveCalendar() {
  ...
  const holidayMap = useMemo(() => {
    ...
    for (const h of holidays) {
      const [holidayYear, holidayMonth, holidayDay] = String(h.date).slice(0, 10).split('-').map(Number)
  ...
  const leaveMap = useMemo(() => {
    ...
    for (const leave of leaves) {
      const fromDate = new Date(`${String(leave.fromDate).slice(0, 10)}T00:00:00`)
      ...
      const cursor = new Date(Math.max(fromDate.getTime(), monthStart.getTime()))
    ...
    <div className="card-surface">
      ...
      <div className="px-5 pb-5 space-y-4">
        ...
        <div className="rounded-xl border border-navy/10 dark:border-white/10 bg-white dark:bg-[var(--bg-secondary)] overflow-hidden">
          {loading && <div className="p-3 text-xs font-bold text-navy/50 dark:text-white/50">Loading approved leaves...</div>}
          {error && <div className="p-3 text-xs font-bold text-red-600 bg-red-50 dark:bg-red-900/30">{error}</div>}
                    ...
                    <>
                      ...
                      <div className="mt-1 space-y-0.5">
                        ...
                        {dayLeaves.slice(0, 2).map((leave) => (
                          <div key={leave.id} title={`${leave.employeeName} · ${leave.leaveTypeName}`} className="text-[9px] px-1.5 py-0.5 rounded bg-amber-50 dark:bg-amber-900/30 text-amber-800 truncate font-bold">
                            {leave.employeeName} · {leave.leaveTypeName}
                          </div>
                        ))}
                        {dayLeaves.length > 2 && <div className="text-[9px] font-bold text-navy/50 dark:text-white/50">+{dayLeaves.length - 2} more</div>}
                      </div>
                    </>
                  )}
                </div>
              ...
              {holidays.map((h) => (
                <div key={h.id} className="flex items-center gap-2 px-2 py-1 rounded-lg bg-white dark:bg-navy-dark/60 border border-navy/5 dark:border-white/5">
                  ...
                  <span className="text-[10px] text-navy/70 dark:text-white/70">
                    ...
                    <span className="text-navy/40 dark:text-white/40"> — {new Date(`${h.date}T00:00:00`).toLocaleDateString()}</span>
                  </span>
                </div>
              ))}
            </div>
