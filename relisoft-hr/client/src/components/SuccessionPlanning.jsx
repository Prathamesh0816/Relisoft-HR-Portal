...
export default function SuccessionPlanning() {
  ...
  useEffect(() => {
    getSuccessionPlanning().then((d) => setResilience({ succession: Array.isArray(d) ? d : (d.succession || []), loading: false })).catch(() => setResilience({ loading: false }))
  }, [])
          ...
          {resilience.succession.map((s, i) => (
            <div key={s.id || i} className="p-4 rounded-xl border border-navy/10 dark:border-white/10 bg-white dark:bg-[var(--bg-secondary)]">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <div className="font-bold text-navy dark:text-white text-sm">{s.employeeName || s.fullName || s.name}</div>
                  <div className="text-xs text-muted mt-1">{s.team || s.department} · {s.jobRole || s.role}</div>
                </div>
                <div className="flex items-center gap-3 text-xs">
                  <span className="text-muted">Tenure: {s.tenureYears ?? s.tenure ?? 0}y</span>
                  <span className="text-muted">Engagement: {s.engagementScore ?? s.engagement ?? 0}</span>
                  <span className="text-muted">Backup: {s.hasBackup ? 'Yes' : 'No'}</span>
                  <span className={`px-3 py-1 rounded-full text-xs font-bold ${s.readinessScore >= 80 || s.readiness >= 80 || s.readiness === 'Ready' ? 'bg-emerald-50 text-emerald-700' : (s.readinessScore >= 50 || s.readiness >= 50) || s.readiness === 'Developing' ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700'}`}>
                    {typeof s.readinessScore === 'number' ? `${s.readinessScore}%` : typeof s.readiness === 'number' ? `${s.readiness}%` : (s.readinessLevel || s.readiness || 'At Risk')}
                  </span>
                </div>
              </div>
            </div>
