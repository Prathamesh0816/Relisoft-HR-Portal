...
export default function WorkforceReadiness() {
  ...
  const teams = readiness.teamReadiness || readiness.teams || []
    ...
    <div className="card-surface p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="p-5 rounded-xl border border-navy/10 dark:border-white/10 bg-white dark:bg-[var(--bg-secondary)] text-center">
          <div className="text-4xl font-bold text-navy dark:text-white">{readiness?.readinessScore ?? readiness?.overallScore ?? 0}%</div>
          <div className="text-xs text-muted mt-1 font-bold">Overall Readiness</div>
        </div>
        <div className="p-5 rounded-xl border border-navy/10 dark:border-white/10 bg-white dark:bg-[var(--bg-secondary)] text-center">
          <div className="text-4xl font-bold text-navy dark:text-white">{readiness?.totalActiveProjects ?? readiness?.activeProjects ?? 0}</div>
      ...
      <div className="space-y-3">
        {(readiness?.teamReadiness || readiness?.teams || []).map((team, i) => (
          <div key={i} className="p-4 rounded-xl border border-navy/10 dark:border-white/10 bg-white dark:bg-[var(--bg-secondary)]">
            <div className="flex items-center justify-between gap-4">
              <div>
                <div className="font-bold text-navy dark:text-white text-sm">{team.team || team.name}</div>
                <div className="text-xs text-muted mt-1">Capacity: {team.capacityUtilization ?? 0}% · Projects: {team.activeProjCount ?? team.activeProjects ?? 0} · Contract Value: ₹{(team.contractValue ?? readiness?.totalContractValue ?? 0).toLocaleString('en-IN')}</div>
              </div>
              <span className="text-lg font-bold text-navy dark:text-white">{team.readinessPct ?? team.readiness ?? 0}%</span>
            </div>
          </div>
        ))}
      </div>
