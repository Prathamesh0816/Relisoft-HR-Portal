...
public class VisitorController : ControllerBase
{
    ...
    public VisitorController(AppDbContext db, NotificationHelper notif, ILogger<VisitorController> logger)
    {
        _db = db;
        _notif = notif;
        _logger = logger;
    ...
    public async Task<ActionResult> GetVisitors([FromQuery] string? status, [FromQuery] string? date)
    {
        ...
        {
            _logger.LogError(ex, "Error fetching visitors");
            return StatusCode(500, new { message = "Error fetching visitors." });
        }
    }
    ...
    public async Task<ActionResult> CreateVisitor([FromBody] Visitor req)
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500, new { message = "Error creating visitor." });
        }
    }
    ...
    public async Task<ActionResult> GetTodayVisitors()
    {
        ...
        {
            _logger.LogError(ex, "Error fetching today's visitors");
            return StatusCode(500, new { message = "Error fetching today's visitors." });
        }
    }
}
